...
- **`Paradise_EW_Admin::$widget_registry` / `$feature_registry` static properties converted to static methods** (`widget_registry()` / `feature_registry()`). PHP forbids function calls (`__()`) in static property initialisers, so the registries had to move to methods to be translatable. All eight in-class access points updated. External access via `get_widget_registry()` / `get_feature_registry()` is unchanged — the public API is stable.
- **`Location %d` shortcode-options string** in `Paradise_Site_Info::get_location_select_options()` now carries a `/* translators: */` comment so translators understand the `%d` placeholder.
